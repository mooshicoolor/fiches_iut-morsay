...
\begin{verbatim}
<type_de_retour> (*<nom_pointeur_fonction>)(<liste_des_types_de_paramètres>);
\end{verbatim}
La déclaration d'un pointeur de fonction spécifie le prototype de la fonction/procédure que l'on souhaite.
\newline Si je souhaite une procédure qui ne prend aucun paramètre je déclare mon pointeur de la façon suivante :
\begin{verbatim}
void (*<nom_pointeur_procédure>)(void)
\end{verbatim}
\textbf{Attention}, n'oubliez pas les parenthèses sinon la syntaxe est mauvaise.
\newline Allons plus loin maintenant
\begin{verbatim}
...
int main()
{
    void (*pointeurProcedure) (void); // je déclare un pointeur de procédure sans paramètre
    pointeurProcedure = maProcedure; /*la variable maProcedure, qui est effectivement le nom la procedure définis
                                      plus haut, contient l'adresse de la procedure maProcedure. Du coup
                                      mon pointeur récupère bien une adresse */
    pointeurProcedure = fonction; /* Cette affectation ne marche pas. Il s'agit du même principe que l'affectation
                                    d'un entier dans une variable de type caractère, cela ne marche pas à moins d'utilser un cast. Mais là je vous conseille pas de vous aventurer dans les cast
                                    de pointeur de fonction*/
    pointeurProcedure(void); // ici j'appelle la fonction sur laquelle pointe mon pointeur
}
\end{verbatim}
...
\item{\emph{fonc} représente un \textbf{pointeur de fonction} - comme on l'a vu juste au dessus, étonnnant non? la fonction qu'on doit envoyer en paramètre doit donc renvoyer \textbf{un pointeur sans type (entre-autre une adresse)} et prendre en paramètre un autre \textbf{pointeur sans type} représentés donc par \textit{void*}. Les pointeur sans type permettent d'envoyer n'importe quelle variable, cependant il faut bien faire attention de caster la variable s'il ne s'agit pas d'un pointeur sans type, exemple : \textit{(void*)&unEntier} ;}
\item{\emph{arg} représente l'argument que l'on doit passer à la fonction qu'on envoie, il faut donc que ce soit un pointeur sans type. C'est donc le paramètre que va utiliser le thread pour la fonction donnée précédemment;}
\end{itemize}
 ...
 *  Comme vous le voyez je fais bien attention à respecter le prototype considéré par \textit{pthread\_create()} */
