...
\subsection{Rediriger des flots vers des tubes}
Vous vous souvenez de \emph{dup2( )}? Vous savez, le cours ou vous avez \sout{rien} tout compris? Eh ben on va en reparler. 	
\paragraph{}
\emph{dup2( )} se présente de la manière suivante:
\begin{verbatim}
int dup2(int fdSource, int fdCible);
\end{verbatim}
En fait, \emph{dup2( )} permet de copier le file descriptor en deuxième argument dans le premier. Par exemple, la commande suivante, en reprenant notre tube \emph{desc}:
\begin{verbatim}
dup2(desc[0], 0);
\end{verbatim}
permet d'envoyer sur \emph{stdin} tout ce qui sort de notre tube. De même, la commande suivante:
\begin{verbatim}
dup2(desc[1],1);
\end{verbatim}
permet d'envoyer tout ce qui sort de \emph{stdout} dans notre tube.
