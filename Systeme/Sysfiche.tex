...
Basé sur \textit{Programmation système en C sous Linux} par \textsc{Christophe Blaess}. C'est un \textit{WIP}, donc encore (et certainement pour toujours) imparfait.\\
Écrit avec \LaTeX{}  qui, selon Mme.Compost, ne tourne \textbf{que} sur \emph{mac}. soz.\\
...
Un programme peut mettre fin à son execution soit de manière \emph{normale} (abandon par l'utilisateur, tâche finie) ou soit, vous l'aurez deviné, de manière \emph{anormale} (bug, crash, tout ce que vous voulez).
...
\chapter{Gestion de fichiers et tout ça.}
...
\item{\textbf{Les fichiers spéciaux}, qui eux mêmes existent en deux type: les fichiers caractères, qui représentent les périphériques d'entrée-sorties (par exemple clavier, allez voir dans \emph{/dev/}), et les fichiers bloc, qui représentent tout ce qui est disque et support de stockage.}
...
\\
\\
Oui merci tg l'exercice. Tout d'abord, notez bien les informations en gras, ce sont les infos les plus importantes. Vu que 1 octet = 8 bits, une adresse tient sur \textbf{4 bits}. \\ 
...
Attention: les descripteurs 0, 1 et 2 sont réservés pour 3 fichiers très spéciaux, respectivement \emph{stdin}, \emph{stdout} et enfin \emph{stderr}.\\
Enfin, le système d'exploitation possède une table commune à tous les processus; c'est la \emph{Table des fichiers ouverts}.

Je continuerais plus tard bicoz.
\end{document}