...
\begin{verbatim}
int pthread_create(pthread_t *thread, NULL, void *(*fonc), void *arg);
\end{verbatim}
Regardons tranquillement les paramètres et leur rôle:
\begin{itemize}
\item{\emph{thread} représente un pointeur (ou une adresse) sur un objet de type \emph{pthread\_t};}
\item{\emph{fonc} représente un pointeur sur un pointeur (wow) de fonction; }
\item{\emph{arg} représente un argument passé pour la fonction \footnote{je vais faire quelques recherches dessus};}
\end{itemize}
Regardons maintenant un exemple:
\begin{lstlisting}
#include<pthread.h>

void *fonction_thread(void *inutile)
{
	printf("coucou, tu veux voir mon pthread_t?????: %d",pthread_self());
	pthread_exit(NULL);
}
...
int main(int argc, char **argv)
{
	...
	pthread_create(&randomThread, NULL, &fonction_thread, NULL);
