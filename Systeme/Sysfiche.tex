...
\item{\emph{attr} représente le ou les attribut du thread, on peut voir ça comme les options de la fonction open(); }
\item{\emph{fonc} représente un pointeur de fonction, ce type de variable permet d'appeler une fonction, le premier \emph{void*} représente ce que la fonction renvoie ici un pointeur sans type, puis \emph{(*fonc)} est le nom de l'attribut, l'étoile fais partie de la syntaxe pour déclarer un pointeur de fonction, enfin \emph{(void *)} représente les attributs de la fonction, ici donc, on un pointeur sans type comme attribut. Ce sera donc la fonction qui sera exécuter par le thread; }
...
void *fonction_thread(void *attr)
{
    ...
    if(attr!=NULL) // on regarde si le pointeur pointe bien sur une case mémoire
    {
        printf("l'attribut en string",(char*)attr); // l'attribut considéré comme une chaine de caractère
        printf("l'attribut en int",*((int*)attr)); // l'attribut considéré comme un pointeur d'entier
    }
    int retour=0;
	pthread_exit((void*)&retour); // je quitte et je renvoie l'adresse de ma variable retour en faisant 
...
int main(int argc, char **argv)
{
	...
	pthread_t* fredId;
    int param = 5;
	pthread_create(fredId, NULL, fonction_thread, (void*)&param); // lorsque je passe un pointeur de fonction en paramètre je
    // passe le nom de la fonction, ici je peux faire le malin parce que la signature de
    // fonction_thread correspond à celle demandé par la signature da la fonction pthread_create
    // j'envoie aussi l'adresse de param en le considérant comme une adresse sans type

	printf("Maintenant j'attends la fin du thread");
    int retour;
	pthread_join(fredId, (void *)retour); // et hop je récupère la valeur de retour	
	exit(retour);
...
\end{verbatim}
avec thread l'identifiant du thread que vous avez envie d'arrêter. \textbf{Cependant} Ne pensez pas que votre fils va s'arrêter sans rien faire, bon par defaults c'est ce qu'il fait mais de façon preque volentaire. Voyons cela.

Je suis un Thread anarchiste et souhaite modifier mon comportement lorsqu'on me demande de m'arreter. J'utilise alors la fonction
\begin{verbatim}
int pthread_setcancelstate(int etat, int *etat_ancien);
\end{verbatim}
Donc comme avec la fonction open, je peux spécifier un ou plusieurs options avec des pipes, pour le paramètre etat :
\begin{itemize}
\item{ Comportement à la reception du signal : PTHREAD_CANCEL_DISABLE pour n'avoir rien à carrer du signal et
PTHREAD_CANCEL_ENABLE pour faire l'enfant modèle}
\item{ Comportement si je récupère le signal : PTHREAD_CANCEL_ASYCHRONOUS pour n'accepter le signal qu'à certains point de contrôle, c'est à dire quand j'utilise la fonction  pthread_join, pthread_testcancel ou pthread_cond_wait. Et PTHREAD_CANCEL_DEFERRED si j'accepte sans attendre le signal. }
\end {itemize}
\begin{dialogue}
...
\speak{...} nous interrompons cette fiche à cause d'un petit problème technique, nous nous reverons plus tard
